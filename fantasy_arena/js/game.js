...
function playSpellFx(card) {
  return new Promise(resolve => {
    ...
    if (!layer) {
      ...
      layer.id = "spellFx";
      layer.innerHTML = '<div class="fx-veil"></div><div class="fx-card" id="fxCard"></div><div class="fx-stage" id="fxStage"></div><div class="fx-name" id="fxName"></div>';
      document.body.appendChild(layer);
    }
    const stage = document.getElementById("fxStage");
    const name = document.getElementById("fxName");
    ...
    setTimeout(() => { if (fxCard) fxCard.classList.add("out"); }, 1400);
    setTimeout(() => {
      buildSpellFx(stage, kind, card);
      if (kind === "earthquake" && game) game.classList.add("quake");
      try { Sfx.spell(kind); } catch (e) {}
    }, 1800);
    setTimeout(() => {
...
function buildSpellFx(stage, kind, card) {
  const elem = (card && (card.tribe || card.element)) || "earth";
  const palette = {
    fire: "ember", water: "frost", wind: "bolt", earth: "rock",
    light: "holy", dark: "shade", 불: "ember", 물: "frost", 바람: "bolt",
    땅: "rock", 빛: "holy", 암흑: "shade"
  };
  const ring = document.createElement("div");
  ring.className = "cast-ring " + (palette[elem] || "rock");
  ring.style.cssText = "position:absolute;left:50%;top:48%;width:220px;height:220px;margin:-110px 0 0 -110px;border-radius:50%;border:3px solid #ffe08a;box-shadow:0 0 40px #ffe08a88,inset 0 0 30px #fff4;animation:flashPop 1.2s ease-out forwards;";
  stage.appendChild(ring);
  if (palette[elem] === "ember") {
    for (let i=0;i<18;i++) {
      const e = document.createElement("div");
      e.className = "ember";
      e.style.left = (20+Math.random()*60)+"%";
      e.style.top = (40+Math.random()*30)+"%";
  ...
  if (kind === "aoe_enemy" || kind === "burst") {
    const n = (card.spell && card.spell.value >= 3) ? 9 : 6;
    for (let i = 0; i < n; i++) {
      const left = 12 + Math.random() * 76;
      const delay = (i * 0.08).toFixed(2) + "s";
      const land = (28 + Math.random() * 28).toFixed(0) + "vh";
      add("rock", { left: left + "%", top: "8%", animationDelay: delay, ["--land"]: land });
      add("dust", { left: (left - 3) + "%", top: "58%", animationDelay: (0.35 + i * 0.08).toFixed(2) + "s" });
    }
  } else if (kind === "float_def") {
    add("isle", { left: "22%", top: "42%" });
    add("isle", { left: "58%", top: "36%", width: "120px", animationDelay: ".15s" });
  } else if (kind === "seal_giant") {
    add("chain", { left: "18%", top: "40%", width: "64%", transformOrigin: "left center" });
    add("chain", { left: "22%", top: "52%", width: "58%", animationDelay: ".12s" });
    add("chain", { left: "20%", top: "64%", width: "62%", animationDelay: ".24s" });
  } else if (kind === "summon_islands") {
    add("isle", { left: "38%", top: "44%" });
    add("isle", { left: "18%", top: "52%", width: "110px", animationDelay: ".2s" });
    add("isle", { left: "58%", top: "50%", width: "100px", animationDelay: ".35s" });
  } else if (kind === "magnet") {
    add("vortex");
  } else if (kind === "earthquake") {
    add("crack");
    add("crack", { left: "42%", animationDelay: ".1s", transform: "translateX(-50%) rotate(-12deg)" });
    add("crack", { left: "58%", animationDelay: ".18s", transform: "translateX(-50%) rotate(10deg)" });
    for (let i = 0; i < 7; i++) add("rock", { left: (15 + i * 10) + "%", animationDelay: (i * 0.06) + "s" });
  } else if (kind === "sandtrap") {
    add("hand", { left: "28%", top: "48%" });
    add("hand", { left: "48%", top: "42%", animationDelay: ".12s" });
    add("hand", { left: "66%", top: "50%", animationDelay: ".24s" });
  } else if (kind === "maze") {
    add("maze");
  } else if (kind === "tunnel") {
    add("hole");
    ...
    add("twister");
    add("twister", { left: "42%", animationDelay: ".1s", opacity: ".5" });
  } else if (kind === "smash") {
    add("shock");
    add("shock", { animationDelay: ".18s" });
    add("aura");
  } else if (kind === "giant_str") {
    add("aura");
    add("shock");
  } else if (kind === "sandhell") {
    add("sand", { left: "30%", top: "40%" });
    add("sand", { left: "50%", top: "46%", animationDelay: ".1s" });
    add("sand", { left: "62%", top: "38%", animationDelay: ".2s" });
    add("hole");
  } else if (kind === "petrify") {
    add("stoneveil");
    add("aura");
  } else {
    add("aura"); add("shock");
...
const TavernBgm = (() => {
  let ctx = null, master = null, drumsG = null, musicG = null, comp = null;
  let playing = false, timer = 0, bar = 0, nextT = 0, noiseBuf = null;
  const BPM = 118;
  const BEAT = 60 / BPM;
  const BAR = BEAT * 4; // 1.818s, 104마디 ≈ 3분 9초 후 루프
  const LOOP = 104;
  ...
  const N = {
    ...
    D4:293.66, E4:329.63, F4:349.23, G4:392.00, A4:440.00, Bb4:466.16, C5:523.25, D5:587.33, E5:659.25, F5:698.46
  };
  ...
  const bassPat = [
    [N.D2, N.D2, N.D3, N.D2, N.D2, N.A2, N.D3, N.F2],
    [N.Bb2, N.Bb2, N.Bb2, N.F2, N.Bb2, N.D3, N.F3, N.Bb2],
    [N.F2, N.F2, N.C3, N.F2, N.F2, N.A2, N.C3, N.F3],
    [N.G2, N.G2, N.D3, N.G2, N.G2, N.Bb2, N.D3, N.G3]
  ];
  const bassDark = [
    [N.D2, 0, N.D2, 0, N.A2, 0, N.D3, N.D2],
    [N.Bb2, 0, N.Bb2, N.F2, 0, N.Bb2, 0, N.F2],
    [N.F2, 0, N.F2, 0, N.C3, 0, N.F2, N.A2],
    [N.A2, 0, N.A2, N.E3, 0, N.A2, 0, N.C3]
  ];
  // 8분음표 멜로디 (마디당 8음). 0 = rest
  const melA = [
    N.D4, N.F4, N.A4, N.G4, N.F4, 0, N.D4, N.A3,
    N.Bb3, N.D4, N.F4, N.A4, N.G4, N.F4, N.D4, 0,
    N.F4, N.A4, N.C5, N.A4, N.F4, N.G4, N.A4, N.F4,
    N.G4, N.Bb4, N.A4, N.G4, N.F4, N.D4, N.E4, N.F4
  ];
  const melB = [
    N.A4, N.D5, N.C5, N.A4, N.G4, N.F4, N.G4, N.A4,
    N.Bb4, N.A4, N.G4, N.F4, N.D4, N.F4, N.G4, 0,
    N.A4, N.C5, N.D5, N.C5, N.A4, N.G4, N.F4, N.A4,
    N.G4, N.F4, N.D4, N.C4, N.D4, N.F4, N.A4, N.D5
  ];
  const melC = [
    N.D5, N.A4, N.F5, N.E5, N.D5, N.C5, N.A4, N.D5,
    N.Bb4, N.D5, N.F5, N.E5, N.D5, N.C5, N.Bb4, N.A4,
    N.C5, N.A4, N.F5, N.D5, N.C5, N.A4, N.G4, N.A4,
    N.D5, N.F5, N.E5, N.D5, N.A4, N.F4, N.G4, N.A4
  ];
  const melD = [
    N.F4, 0, N.D4, 0, N.A4, 0, N.F4, N.D4,
    N.G4, 0, N.D4, N.F4, 0, N.A4, 0, N.G4,
    N.A4, 0, N.F4, 0, N.C5, N.A4, 0, N.F4,
    N.D5, N.A4, N.F4, N.G4, N.A4, 0, N.D4, 0
  ];

  function sectionOf(n) {
    const m = n % LOOP;
    if (m < 8) return "intro";
    if (m < 24) return "drive";
    if (m < 40) return "lift";
    if (m < 56) return "dark";
    if (m < 72) return "hero";
    if (m < 88) return "peak";
    if (m < 96) return "break";
    return "outro";
  }
  ...
  function ensure() {
    ...
    drumsG = ctx.createGain(); drumsG.gain.value = 0.9;
    musicG = ctx.createGain(); musicG.gain.value = 0.85;
  ...
  function envGain(t, a, hold, rel, vol, dest) {
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(Math.max(vol, 0.0002), t + a);
    g.gain.exponentialRampToValueAtTime(Math.max(vol * 0.7, 0.0002), t + a + hold);
  ...
  function kick(t, vol) {
    const g = envGain(t, 0.004, 0.04, 0.22, vol, drumsG);
    const o = ctx.createOscillator();
    o.type = "sine";
    ...
    const cg = envGain(t, 0.001, 0.01, 0.03, vol * 0.25, drumsG);
    click.type = "square";
  ...
  function snare(t, vol) {
    ...
    bp.Q.value = 0.9;
    const g = envGain(t, 0.003, 0.03, 0.12, vol, drumsG);
    src.connect(bp); bp.connect(g);
    src.start(t); src.stop(t + 0.18);
    const o = ctx.createOscillator();
    const og = envGain(t, 0.002, 0.02, 0.1, vol * 0.35, drumsG);
    o.type = "triangle";
  ...
  function hat(t, vol, open) {
    ...
    const g = envGain(t, 0.001, open ? 0.06 : 0.015, open ? 0.08 : 0.04, vol, drumsG);
    src.connect(hp); hp.connect(g);
    src.start(t); src.stop(t + (open ? 0.16 : 0.07));
  }

  function tom(t, freq, vol) {
    const g = envGain(t, 0.004, 0.04, 0.16, vol, drumsG);
    osc("sine", freq, t, 0.22, g);
    const o = ctx.createOscillator();
    o.type = "sine";
    o.frequency.setValueAtTime(freq, t);
  ...
  function bassNote(freq, t, dur, vol) {
    if (!freq) return;
    const g = envGain(t, 0.02, dur * 0.45, dur * 0.4, vol, musicG);
    osc("sawtooth", freq, t, dur, g);
    osc("square", freq * 0.5, t, dur, g);
    const lp = ctx.createBiquadFilter();
    lp.type = "lowpass";
    lp.frequency.setValueAtTime(420, t);
    lp.frequency.linearRampToValueAtTime(280, t + dur);
    // reconnect through lp: rebuild
  }

  function bassNote2(freq, t, dur, vol) {
    if (!freq) return;
    const lp = ctx.createBiquadFilter();
    lp.type = "lowpass";
    lp.frequency.value = 380;
    lp.connect(musicG);
    const g = envGain(t, 0.018, dur * 0.5, dur * 0.35, vol, lp);
    osc("sawtooth", freq, t, dur + 0.02, g);
    osc("sine", freq, t, dur + 0.02, g);
  }
  ...
  function stab(freqs, t, dur, vol) {
    ...
    lp.connect(musicG);
    const g = envGain(t, 0.02, dur * 0.35, dur * 0.5, vol, lp);
    freqs.forEach((f, i) => {
      osc(i === 0 ? "sawtooth" : "triangle", f, t, dur, g);
    });
  }

  function leadNote(freq, t, dur, vol) {
    if (!freq) return;
    const lp = ctx.createBiquadFilter();
    lp.type = "lowpass";
    ...
    lp.connect(musicG);
    const g = envGain(t, 0.015, dur * 0.4, dur * 0.45, vol, lp);
    osc("sawtooth", freq, t, dur + 0.02, g);
    osc("triangle", freq * 2, t, dur + 0.02, g);
  }

  function padChord(freqs, t, dur, vol) {
    const lp = ctx.createBiquadFilter();
    lp.type = "lowpass";
    lp.frequency.value = 900;
    lp.connect(musicG);
    const g = envGain(t, 0.18, dur * 0.55, dur * 0.35, vol, lp);
    freqs.forEach(f => osc("sawtooth", f, t, dur + 0.05, g));
  }

  function scheduleBar(n, t) {
    const sec = sectionOf(n);
    const ci = n % 4;
    const chord = (sec === "dark" || sec === "break")
      ? chords[ci === 3 ? 4 : ci]
      : chords[ci];
    const eighth = BAR / 8;
    const sixteenth = BAR / 16;

    const drumOn = sec !== "intro";
    const fullKit = sec === "drive" || sec === "lift" || sec === "hero" || sec === "peak";
    const halfKit = sec === "dark" || sec === "break" || sec === "outro";

    padChord(chord, t, BAR * 0.98, sec === "intro" ? 0.05 : sec === "dark" ? 0.035 : 0.03);

    if (drumOn) {
      const kVol = sec === "peak" ? 0.22 : sec === "hero" ? 0.2 : 0.16;
      const sVol = sec === "peak" ? 0.16 : 0.12;
      kick(t, kVol);
      kick(t + BEAT * 2, kVol * 0.92);
      if (fullKit) {
        snare(t + BEAT, sVol);
        snare(t + BEAT * 3, sVol);
        if (sec === "peak" || sec === "hero") kick(t + BEAT * 2.5, kVol * 0.45);
        if (n % 4 === 3) {
          tom(t + BEAT * 3.25, 180, 0.1);
          tom(t + BEAT * 3.5, 140, 0.12);
          tom(t + BEAT * 3.75, 100, 0.14);
        }
      } else if (halfKit) {
        snare(t + BEAT * 2.5, sVol * 0.7);
      }
      for (let i = 0; i < 8; i++) {
        const open = fullKit && i === 7;
        hat(t + i * eighth, open ? 0.045 : 0.028, open);
      }
      if (fullKit) {
        for (let i = 0; i < 16; i += 2) {
          if (i % 4 !== 0) hat(t + i * sixteenth, 0.016, false);
        }
      }
    }

    const bpat = (sec === "dark" || sec === "break") ? bassDark[ci] : bassPat[ci];
    const bVol = sec === "intro" ? 0.04 : 0.07;
    bpat.forEach((f, i) => {
      if (f) bassNote2(f, t + i * eighth, eighth * 0.92, bVol);
    });

    if (sec !== "intro" && sec !== "outro") {
      stab(chord, t, BAR * 0.22, 0.03);
      if (fullKit) stab(chord, t + BEAT * 2, BAR * 0.16, 0.024);
    }

    let mel = melA;
    if (sec === "lift") mel = melB;
    else if (sec === "hero" || sec === "peak") mel = melC;
    else if (sec === "dark" || sec === "break") mel = melD;
    else if (sec === "intro") mel = melA.map((x, i) => (i % 2 === 0 ? x : 0));
    else if (sec === "outro") mel = melC.map((x, i) => (i < 4 ? x : 0));

    const leadVol = sec === "hero" || sec === "peak" ? 0.055 : sec === "intro" ? 0.028 : 0.042;
    const slice = mel.slice((n % 4) * 8, (n % 4) * 8 + 8);
    slice.forEach((f, i) => {
      if (!f) return;
      const dur = eighth * ((i === 7 || !slice[i + 1]) ? 1.35 : 0.95);
      leadNote(f, t + i * eighth, dur, leadVol);
      if (sec === "peak") leadNote(f * 1.5, t + i * eighth + 0.02, dur * 0.8, leadVol * 0.22);
    });
  }
  ...
  return { start, stop, toggle, isOn: () => playing };
})();
