...
async function composeCardFace(c, opts={}) {
  ...
  paintNumber(ctx, String(c.cost ?? 0), W*0.1516, H*0.103, Math.round(H*0.070));
  const paintFrameStats = c.type === "minion" || (c.type === "item");
  if (paintFrameStats) {
    const hp = opts.hp != null ? opts.hp : c.hp;
    paintNumber(ctx, String(c.atk ?? 0), W*0.1499, H*0.9032, Math.round(H*0.066));
    paintNumber(ctx, String(c.def ?? 0), W*0.5008, H*0.9032, Math.round(H*0.066));
    paintNumber(ctx, String(hp ?? 0), W*0.8654, H*0.9032, Math.round(H*0.066));
...
async function paintStatCoins(ctx, c, W, H) {
  ...
  const cy = H * 0.972;
  const slots = [
    [W * 0.1499, parseCoin(c.atkC)],
    [W * 0.5008, parseCoin(c.defC)],
    [W * 0.8654, parseCoin(c.hpC)]
  ];
...
function faceCacheKey(c, opts) {
  return ["v93statNudge", c.id, c.type || "", c.tribe || "", c.cost, c.atk, c.def, c.atkC, c.defC, c.hpC, opts && opts.hp != null ? opts.hp : c.hp, c.name, c.itemWorn ? "eq" : "", (c.equippedItem && c.equippedItem.id) || ""].join("|");
}
